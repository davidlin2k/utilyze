...
.PHONY: all clean test-native-unit test-native-smoke test-native-smoke-full dist-tarball dist-tarball-docker utlz
...
utlz: $(TARGET)
	@mkdir -p internal/ffi/sampler/embedded/
	@mkdir -p $(dir $(TARGET))
	cp $(TARGET) internal/ffi/sampler/embedded/libutlz_sampler.so.$(VERSION_MAJOR)
	go build \
		-trimpath \
		-ldflags="-s -w -X github.com/systalyze/utilyze/internal/version.VERSION=$(VERSION) -X github.com/systalyze/utilyze/internal/ffi/sampler.SHA256SUM=$$(sha256sum internal/ffi/sampler/embedded/libutlz_sampler.so.$(VERSION_MAJOR) | cut -d' ' -f1) -X github.com/systalyze/utilyze/internal/version.REPO=$(REPO)" \
		-o dist/utlz-linux-$(ARCH) \
		cmd/main.go
